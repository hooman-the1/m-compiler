import { Car, InsertActionScenaio } from "../interfaces/interfaces.js";
...
export default class Insert extends Database{
    ...
    async insertCarsIntoDatabaseTmp(brandCars: Car[]){
        const connect = await this.createConnect(this.carsDBName);
        const brandName = brandCars[0].collectionName.replace(/[0-9]/g,'');
        const colExists = await (await connect!.dbo.listCollections().toArray()).findIndex((item: any) => item.name === brandName) !== -1;
        if(!colExists){
    ...
    private async createCollectionTmp(){}
    private async createDocumentTmp(){}
    private async createVariantTmp(){}
    private async updateTmp(){}

    private async insertCarTmp(dbo: any, car: Car){
        try{
    ...
    private async tryInsertTmp(dbo: any, car: Car){
        const brandName = car.collectionName.replace(/[0-9]/g,'');
        const lastUpdate = this.date.getCurrentGDate() ;
        const isActive = false;
        const specs = 'test';
        (car as any)['lastUpdate'] = lastUpdate;
        (car as any)['isActive'] = isActive;
